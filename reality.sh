...
function show_help {
  echo ""
  echo "Usage: reality.sh [-t|--transport=tcp|http|grpc|ws|tuic|hysteria2|shadowtls] [-d|--domain=<domain>] [--server=<server>] [--regenerate] [--default]
...
function parse_users_file {
  ...
  if [[ ${#users[@]} -eq 0 ]]; then
    users[Realit]=$(cat /proc/sys/kernel/random/uuid)
    echo "Realit=${users[Realit]}" >> "${path[users]}"
    return 0
  fi
  return 0
}
...
function download_tgbot_script {
  curl -fsSL -m 3 https://raw.githubusercontent.com/aleskxyz/reality/master/tgbot.py -o "${path[tgbot_script]}"
}
...
function tune_kernel {
  cat >/etc/sysctl.d/99-reality.conf <<EOF
fs.file-max = 200000
...
EOF
  sysctl -qp /etc/sysctl.d/99-reality.conf >/dev/null 2>&1 || true
}
